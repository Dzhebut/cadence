...
public class JobApi {
    ...
    try {
      ...
      String xml =
          "<flow-definition plugin=\"workflow-job@2.32\">\n"
              ...
              + "</flow-definition>";
      // 创建 Job
      jenkinsServer.createJob("demo", xml);
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
    ...
    try {
      ...
      String xml =
          "<flow-definition plugin=\"workflow-job@2.32\">\n"
              ...
              + "</flow-definition>";
      // 创建 Job
      jenkinsServer.updateJob("demo", xml);
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
  ...
  public void getMavenJob() {
    try {
      // 获取 Job 信息
      MavenJobWithDetails job = jenkinsServer.getMavenJob("demo");
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
  ...
  public void getJobConfig() {
    try {
      String xml = jenkinsServer.getJobXml("demo");
  ...
  public void buildJob() {
    try {
      jenkinsServer.getJob("demo").build();
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
  ...
  public void buildParamJob() {
    try {
      /** 例如，现有一个job，拥有一个字符参数"key" 现在对这个值进行设置，然后执行一个输出这个值的脚本 */
      // 设置参数值
      Map<String, String> param = new HashMap<>();
      param.put("key", "hello world!");
      // 执行 build 任务
      jenkinsServer.getJob("demo").build(param);
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
  ...
  public void stopLastJobBuild() {
    try {
      // 获取最后的 build 信息
      Build build = jenkinsServer.getJob("demo").getLastBuild();
      // 停止最后的 build
      build.Stop();
    } catch (IOException e) {
      e.printStackTrace();
